...
class Member extends JFrame {
	...
	public void buttonFunction() {
		button[0].addActionListener(event -> {
			...
			if (!where[0].equals("")) {
				query = query + "movie_title = '" + where[0] + "'";
				f = 1;
			}
			if (!where[1].equals("")) {
				if (f == 1)
			...
			JTable table = sql.MovieSearch(error, query);
...
class SQL {
	...
	public JTable MovieSearch(JLabel error, String sql) {
		Object[] attribute = { "영화번호", "영화명", "상영시간", "상영등급", "감독명", "배우명", "장르", "영화소개", "개봉일" };
